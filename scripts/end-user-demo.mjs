...
import {
  lowRiskBaselineFiles,
  lowRiskChangedFiles,
  mediumRiskBaselineFiles,
  mediumRiskChangedFiles,
  prSafetyBaselineFiles,
  prSafetyRiskyFiles
} from "./fixtures/end-user-demo/repo-fixtures.mjs";
import {
  renderGitHubActionSmokeScript,
  renderMcpClientSmokeScript
} from "./fixtures/end-user-demo/smoke-scripts.mjs";
...
function writeMcpClientSmokeScript({ base, head }) {
  writeFileSync(
    ...
    renderMcpClientSmokeScript({
      ...
      base,
      head,
      clientModuleUrl: pathToFileURL(join(repoRoot, "packages/mcp/node_modules/@modelcontextprotocol/sdk/dist/esm/client/index.js")).href,
      stdioModuleUrl: pathToFileURL(join(repoRoot, "packages/mcp/node_modules/@modelcontextprotocol/sdk/dist/esm/client/stdio.js")).href
    }),
    "utf8"
  );
}
...
function writeGitHubActionSmokeScript({ base, head }) {
  writeFileSync(
    ...
    renderGitHubActionSmokeScript({
      outputPath: actionSmokeReportPath,
      repoRoot,
      workspace: demoRepo,
      base,
      head,
...
function createLowRiskRepo(root) {
  resetDir(root);
  writeFiles(root, lowRiskBaselineFiles());
  initGitRepo(root);
  writeFiles(root, lowRiskChangedFiles());
}
...
function createPrSafetyDemoRepo(root) {
  resetDir(root);
  writeFiles(root, prSafetyBaselineFiles());
  initGitRepo(root);
  const base = gitOutput(root, ["rev-parse", "HEAD"]).trim();

  writeFiles(root, prSafetyRiskyFiles());
