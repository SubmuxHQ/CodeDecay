...
export function parseResilienceArgs(args: string[]): ResilienceOptions {
  const options: ResilienceOptions = { surfaceFiles: [], format: "markdown" };
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (flag === "--experiment") options.experimentFile = value();
      ...
      throwUnknownOption(arg, "resilience");
      continue;
    }
...
function parseFormat(value: string): ResilienceOptions["format"] {
  if (value === "json" || value === "markdown") return value;
  throw new Error(`Invalid resilience format "${value}". Expected json or markdown.`);
}

function parseTarget(value: string): NonNullable<ResilienceOptions["targetKind"]> {
  if (
    value === "unspecified" ||
    value === "fixture-local" ||
  ...
  throw new Error(`Invalid resilience target kind "${value}".`);
}
