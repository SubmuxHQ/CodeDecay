...
export function installFakePlaywright(repo: string): void {
  ...
  writeFile(
    ...
    [
      ...
      "const configDir = configIndex === -1 ? undefined : process.argv[configIndex + 1];",
      "const configuredSpec = configDir ? readdirSync(configDir).find((entry) => entry.endsWith('.spec.ts')) : undefined;",
      "const sourcePath = process.argv.find((arg) => arg.endsWith('.spec.ts')) ?? (configuredSpec ? join(configDir, configuredSpec) : undefined);",
