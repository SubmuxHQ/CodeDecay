...
describe("unified harness planted issue corpus", () => {
  ...
  it("exposes the same canonical corpus consumed by the benchmark command", () => {
    ...
    expect(corpus.scenarios.filter((scenario) => scenario.kind === "positive")).toHaveLength(7);
    expect(corpus.scenarios.filter((scenario) => scenario.kind === "decoy")).toHaveLength(5);
    expect(corpus.scenarios.map((scenario) => scenario.id)).toEqual(
      expect.arrayContaining([
        "one-hop-sqli",
        "indirect-dynamic-sqli",
