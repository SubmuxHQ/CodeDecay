...
export function renderRuntimeEvidenceMarkdown(report: RuntimeEvidenceReport): string {
  ...
  for (const item of report.operations) lines.push(`- **${item.service} ${item.route ?? item.operation}**: ${item.spanCount} span(s), ${item.errorCount} error(s), max ${item.maxLatencyMs}ms; trust \`${item.trust}\`.`);
  lines.push("", "### Correlated Errors", "");
  if (report.errors.length === 0) lines.push("No structured error groups were ingested.", "");
  for (const item of report.errors) lines.push(`- **${item.group}**: ${item.count} event(s) for ${item.service}; trust \`${item.trust}\`; source \`${item.sourceRef}\`.`);
  ...
  lines.push("", "### Safety", "", "- Local artifact ingestion only; no network or command execution.", "- Sensitive attributes, query strings, authorization data, request bodies, tokens, and email addresses are redacted before report assembly.", "- Historical or sampled runtime evidence cannot prove the current tree safe.", "");
  return `${lines.join("\n")}\n`;
}
